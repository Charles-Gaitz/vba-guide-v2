...
Write a macro that counts the records in a column and calculates
the sum of a numeric column, then displays both in a MsgBox.
...
Format the cell: `Range("C5").NumberFormat = "mm/dd/yyyy"`
...
them, calculates the average, and displays it formatted to 3 decimal places.
...
GPR averages formatted to 3 decimal places using Format(value, "0.000").
...
ActiveCell is currently D10. Then paste the code into VBA and run
it to verify your predictions.
...
**Step 3 — Compare**
Open both macros in the VBA Editor. What's different?

**Solution:**
AbsoluteTest uses Range("C5").Select — always goes to C5.
RelativeTest uses ActiveCell.FormulaR1C1 without a Range select —
moves relative to wherever the active cell was when you started recording.
Run RelativeTest with different cells selected and watch where it types.
...
Using the Aggie Advisors data, write a macro that navigates to the
Student Information sheet and finds the first empty row below the
existing data — ready to add a new student.
...
3. Copy the first 10 rows of Aggie Advisors student data into rows 2–11
   (row 12 will be empty — your macro should land there)
...
**Title:** Populate the Next Empty Row
**No hints. No steps. Exam level.**

Use the same Student Information sheet from the Practice Problem
(10 rows of data in rows 2–11, first empty row is row 12).
Your Aggie Advisors applicant data is on Sheet1.

Write a macro that:
1. Navigates to the Applicant Information sheet (Sheet1) and reads the
   first applicant record into variables: StudentID, LastName, FirstName, GPR
2. Navigates to the Student Information sheet
3. Goes to cell A2 (absolute reference only)
4. Uses End(xlDown) and Offset to find the first empty row
5. Populates that row using Offset from ActiveCell — no hardcoded cell addresses:
   StudentID | 35 | U | LastName | FirstName | GPR
